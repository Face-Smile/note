...
![image-20191121164004306](基础知识/image-20191121164004306.png)
...
- 客户端请求服务器包含一个随机同步号`SYN`
- 服务器回复一个确认号(ACK, 值为收到的客户端同步号加一)和一个随机同步号(SYN)给客户端
- 客户端回复一个确认号(ACK, 值为收到的服务器同步号加一), 完成tcp连接
...
![image-20191120202226885](基础知识/image-20191120202226885.png)
...
短连接: 即每次接收到http请求完成数据收发后,会立即关闭tcp连接

长连接: 建立tcp连接后,进行http请求收发数据后,不会立即关闭tcp连接,而是继续等待客户端是否有新的请求,客户端关闭连接后,连接才关闭, 客户端如何知道一个http请求是否完成,是通过响应头中的`Content-Length`的值来判断的
